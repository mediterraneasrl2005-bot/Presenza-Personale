...
import { Clock, LogOut, Users, AlertCircle, Calendar } from 'lucide-react';

// ─── DATI DIPENDENTI (tutti i 32 dal tuo JSON) ───────────────────────────────
...
// Distribuisce le ore sui giorni lavorativi del mese
function generaPresenze(dipendente: any, meseInfo: any): number[] {
  const { giorni, anno, mese } = meseInfo;
  if (dipendente.ore_g === 0) return Array(giorni).fill(0);
  const result = Array(giorni).fill(0);
  let oreRimanenti = dipendente.totale_ore;
  for (let g = 1; g <= giorni; g++) {
    if (oreRimanenti <= 0) break;
    const dow = new Date(anno, mese, g).getDay();
    if (dow === 0 || dow === 6) continue;
    const ore = Math.min(dipendente.ore_g, oreRimanenti);
    result[g - 1] = Math.round(ore * 100) / 100;
    oreRimanenti -= ore;
  }
  return result;
}
...
const CalendarioMensile = ({ dipendente, meseInfo }: { dipendente: any; meseInfo: any }) => {
  const { giorni, anno, mese } = meseInfo;
  const presenze = generaPresenze(dipendente, meseInfo);
  const totOre = presenze.reduce((a, b) => a + b, 0);
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
      ...
      <div className="grid grid-cols-7 gap-0.5">
        ...
        {Array.from({ length: giorni }, (_, i) => i + 1).map((giorno) => {
          const ore = presenze[giorno - 1] ?? 0;
          const dow = new Date(anno, mese, giorno).getDay();
          const isWeekend = dow === 0 || dow === 6;
          return (
            <div key={giorno} title={`${giorno}: ${ore}h`}
              className={`aspect-square rounded flex flex-col items-center justify-center cursor-default
                ${isWeekend ? 'bg-slate-800/20 text-slate-700'
                  : ore > 0 ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
                  : 'bg-rose-500/10 text-rose-500/50 border border-rose-500/20'}`}
            >
              <span className="text-[7px] opacity-50 leading-none">{giorno}</span>
              {!isWeekend && <span className="text-[7px] font-bold leading-none">{ore > 0 ? ore : '—'}</span>}
            </div>
          );
        })}
      </div>
      <div className="flex gap-3 mt-2">
        <div className="flex items-center gap-1">
          ...
          <span className="text-[8px] text-slate-600">Presente</span>
        </div>
        <div className="flex items-center gap-1">
...
export default function App() {
  ...
  const [meseSelIdx, setMeseSelIdx] = useState(13); // default: Febbraio 2026

  const meseInfo = MESI[meseSelIdx];
  const totaleOreGenerale = datiDipendenti.reduce((s, d) => s + d.totale_ore, 0);
  ...
  const handleLogout = () => { setUser(null); setPin(''); setSelectedDip(null); setAdminView('lista'); };
  ...
  if (isAdmin) return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <Card title="Ore Totali Registrate" subtitle={`${totaleOreGenerale.toFixed(0)}h`} icon={Clock} color="bg-emerald-500" />
            ...
            <tbody className="divide-y divide-slate-800">
              {datiDipendenti.map(d => (
                <tr key={d.id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="p-4 text-slate-300 text-xs">{d.ore_g}h</td>
                  <td className="p-4 text-white font-bold text-xs">{d.totale_ore}h</td>
                  <td className="p-4">
                    <span className={`text-[10px] px-2 py-1 rounded ${d.ore_g > 0 ? 'bg-slate-800 text-slate-500' : 'bg-rose-500/10 text-rose-400'}`}>
                      {d.ore_g > 0 ? 'Offline' : 'Inattivo'}
                    </span>
                  </td>
                  <td className="p-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {(selectedDip ? [selectedDip] : datiDipendenti).map(d => (
              <CalendarioMensile key={d.id} dipendente={d} meseInfo={meseInfo} />
            ))}
          </div>
        </div>
      )}
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white p-6">
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 mb-6 flex justify-between items-center">
        <div>
          <p className="text-slate-400 text-xs mb-1">Ore totali registrate</p>
          <p className="text-emerald-400 font-black text-2xl">{user.totale_ore}h</p>
        </div>
        <div className="text-right">
          <p className="text-slate-400 text-xs mb-1">Ore al giorno</p>
          <p className="text-white font-bold text-lg">{user.ore_g}h</p>
        </div>
      </div>
      <div className="space-y-4">
        <button className="w-full h-36 bg-emerald-500 rounded-[2rem] font-black text-2xl shadow-xl shadow-emerald-500/20 active:scale-95 transition-transform">ENTRATA</button>
        <button className="w-full h-36 bg-rose-500 rounded-[2rem] font-black text-2xl shadow-xl shadow-rose-500/20 active:scale-95 transition-transform">USCITA</button>
      </div>
    </div>
  );
}
